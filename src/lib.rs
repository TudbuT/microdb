        ...
        if time - self.last_cache_check > 100 {
            ...
            for item in self.cache.iter_mut() {
                if item.1 .1 && time - item.1 .0 >= self.cache_period {
                    println!("saving allocation {item:?}");
...
impl MicroDB {
    fn internal_new(
        ...
        let inner = Arc::new(Mutex::new(InnerMicroDB {
            cache_period,
            data,
            alloc,
            cache: HashMap::new(),
...
mod test {
    ...
    fn create() {
        let mut db = MicroDB::create("test.dat", "test.alloc", 500, 256).unwrap();
    ...
    fn load() {
        let mut db = MicroDB::new("test.dat", "test.alloc", 500).unwrap();
        assert_eq!(db.get("test").unwrap().unwrap(), vec![40_u8; 400]);
        db.shutdown().unwrap();
    }
    fn delete_val() {
    ...
    fn create_new_val() {
        let mut db = MicroDB::new("test.dat", "test.alloc", 500).unwrap();
        db.set("test2", vec![40; 400]).unwrap();
        assert_eq!(db.get("test2").unwrap().unwrap(), vec![40_u8; 400]);
        db.sync().unwrap();
        db.set("lol2", vec![51; 512]).unwrap();
        assert_eq!(db.get("lol2").unwrap().unwrap(), vec![51_u8; 512]);
        db.sync().unwrap();
        db.shutdown().unwrap();
    }
}
